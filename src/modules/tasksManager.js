...
const tasksManager = (() => {
    ...
    function addStandaloneTask () {
        ...
        let task = new Task (description.value, formatDate(new Date(dueDate.value.replace(/-/g, '/'))), priority, false)
        taskList.push(task)
    ...
    function addProjectTask (e) {
        ...
        if (description.value && dueDate.value){
            ...
            let task = new Task (description.value, formatDate(new Date(dueDate.value.replace(/-/g, '/'))), 0, false)
